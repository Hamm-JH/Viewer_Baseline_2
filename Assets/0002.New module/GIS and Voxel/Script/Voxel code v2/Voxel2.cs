...
namespace Test
{
    public class Voxel2 : MonoBehaviour
    {
        ...
        /// 특정 복셀이 지하에 존재하는가?
        /// </summary>
        /// <param name="center">복셀의 중심점</param>
        /// <returns>true : 지하에 있음</returns>
        ...
        /// <param name="rotation">회전각도</param>
        /// <param name="scale">크기</param>
        /// <param name="depth">복셀 depth</param>
        /// <param name="totalDepth">총 복셀 depth</param>
        /// <param name="_controller">복셀 컨트롤러 </param>
        public void InitVoxel(Vector3 center, Quaternion rotation, Vector3 scale, int depth, int totalDepth, VoxelController _controller)
        {
            ...
            m_data = new Data();
            m_data.center = center; // 중심점 할당
            m_data.scale = scale;   // 크기 할당
            m_data.min = new Vector3(   // 최소벡터 할당
                center.x - scale.x / 2,
                center.y - scale.y / 2,
                center.z - scale.z / 2);
            
            m_data.max = new Vector3(   // 최대벡터 할당
                center.x + scale.x / 2,
                center.y + scale.y / 2,
                center.z + scale.z / 2);

            m_data.rotation = rotation; // 회전값 할당
            m_data.totalDepth = totalDepth; // 뎁스 할당
        ...
        /// <param name="center">중심점</param>
        /// <param name="angleY">Y축 회전 각도</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 복셀 리스트 할당시 재귀로 자식 리스트 싹 순회하는 메서드
        /// </summary>
        /// <param name="near">충돌되지 않은 객체 리스트</param>
        /// <param name="nearCollision">충돌된 객체 리스트</param>
