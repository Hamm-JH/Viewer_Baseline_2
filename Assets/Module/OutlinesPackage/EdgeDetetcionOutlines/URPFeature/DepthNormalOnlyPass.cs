...
public class DepthNormalOnlyPass : ScriptableRenderPass
{
    ...
    public DepthNormalOnlyPass(RenderPassEvent renderPassEvent, LayerMask outlinesLayerMask,bool useDepthMask, bool useAlphaCutoff , float cutoff )
    {
        filteringSettings = new FilteringSettings(RenderQueueRange.all, outlinesLayerMask);

        normalsMaterial = new Material(Shader.Find("Hidden/NormalsDepth"));
        normalsMaterial.SetFloat("_UseDepthMask", useDepthMask ? 1 : 0);
        normalsMaterial.SetFloat("_UseAlphaCutoff", useAlphaCutoff ? 1 : 0);
        normalsMaterial.SetFloat("_Cutoff", cutoff);
