using UnityEngine;
using UnityEngine.Rendering;

namespace Bearroll.UltimateDecals {

	public static class LightProbeUtility {

		public static void SetAmbientSHCoefficients() {

			SphericalHarmonicsL2 sh;
			LightProbes.GetInterpolatedProbe(Vector3.up * 100000, null, out sh);
			
			// Constant + Linear
			for (var i = 0; i < 3; i++)
				Shader.SetGlobalVector(_idSHA[i], new Vector4(
					sh[i, 3], sh[i, 1], sh[i, 2], sh[i, 0] - sh[i, 6]
				));

			// Quadratic polynomials
			for (var i = 0; i < 3; i++)
				Shader.SetGlobalVector(_idSHB[i], new Vector4(
					sh[i, 4], sh[i, 6], sh[i, 5] * 3, sh[i, 7]
				));

			// Final quadratic polynomial
			Shader.SetGlobalVector(_idSHC, new Vector4(
				sh[0, 8], sh[2, 8], sh[1, 8], 1
			));

		}

		// Set SH coefficients to MaterialPropertyBlock
		public static void SetSHCoefficients(Vector3 position, MaterialPropertyBlock properties) {
			SphericalHarmonicsL2 sh;
			LightProbes.GetInterpolatedProbe(position, null, out sh);

			// Constant + Linear
			for (var i = 0; i < 3; i++)
				properties.SetVector(_idSHA[i], new Vector4(
					sh[i, 3], sh[i, 1], sh[i, 2], sh[i, 0] - sh[i, 6]
				));

			// Quadratic polynomials
			for (var i = 0; i < 3; i++)
				properties.SetVector(_idSHB[i], new Vector4(
					sh[i, 4], sh[i, 6], sh[i, 5] * 3, sh[i, 7]
				));

			// Final quadratic polynomial
			properties.SetVector(_idSHC, new Vector4(
				sh[0, 8], sh[2, 8], sh[1, 8], 1
			));
		}

		// Set SH coefficients to Material
		public static void SetSHCoefficients(
			Vector3 position, Material material
		) {
			SphericalHarmonicsL2 sh;
			LightProbes.GetInterpolatedProbe(position, null, out sh);

			// Constant + Linear
			for (var i = 0; i < 3; i++)
				material.SetVector(_idSHA[i], new Vector4(
					sh[i, 3], sh[i, 1], sh[i, 2], sh[i, 0] - sh[i, 6]
				));

			// Quadratic polynomials
			for (var i = 0; i < 3; i++)
				material.SetVector(_idSHB[i], new Vector4(
					sh[i, 4], sh[i, 6], sh[i, 5] * 3, sh[i, 7]
				));

			// Final quadratic polynomial
			material.SetVector(_idSHC, new Vector4(
				sh[0, 8], sh[2, 8], sh[1, 8], 1
			));
		}

		static int[] _idSHA = {
			Shader.PropertyToID("UD_SHAr"),
			Shader.PropertyToID("UD_SHAg"),
			Shader.PropertyToID("UD_SHAb")
		};

		static int[] _idSHB = {
			Shader.PropertyToID("UD_SHBr"),
			Shader.PropertyToID("UD_SHBg"),
			Shader.PropertyToID("UD_SHBb")
		};

		static int _idSHC =
			Shader.PropertyToID("UD_SHC");

	}

}