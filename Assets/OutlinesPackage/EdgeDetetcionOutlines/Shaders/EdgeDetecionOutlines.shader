Shader "Hidden/EdgeDetectionOutlines"
{
	Properties
	{
		// [Toggle(DEBUG)]DEBUG("DEBUG", Float) = 0
			 ...
			 _SteepAngleThreshold("_SteepAngleThreshold",Range(-2,1)) = 1
			 _SteepAngleMultiplier("_SteepAngleMultiplier",Range(1,5)) = 1
	 ...
	 SubShader
    {
		...
		CBUFFER_END
            ...
            v2f vert (appdata v)
            {
                ...
                o.uv = v.uv;
				o.viewSpaceDir = mul(UNITY_MATRIX_I_P,o.vertex).xyz;
                return o;
            }
            ...
            float4 frag (v2f input) : SV_Target
            {
				...
				float NdotVTrans = 1 - smoothstep(_SteepAngleThreshold,2,NdotV) * _SteepAngleMultiplier + 1;
				//return NdotVTrans * edgeDepth;

				edgeDepth*=NdotVTrans;
